...
Secondly, the \gls{asbc} contains articles from the year of 1981 to 2007. The corpus is well-balanced across genres and carefully segmented and POS tagged. In addition, not only are articles in \gls{asbc} temporal-labeled, but the texts are also carefully segmented, which makes it an ideal input data for word vector representations. Therefore, the choice of \gls{ctext} and \gls{asbc} suits the language settings for this study. The overview of type and token counts of texts from the time-sliced corpora is as \tref{tab:token_type_counts}
